...
void CFileEditCtrl::ButtonClicked()
{
    ...
    // Clear filter. so this is not set again if the button is pressed a second time. 
    // if not. then a error dialog will be shown (Vista/Win7+ only )
    if(m_pCFileDialog)
    {
      m_pCFileDialog->m_pOFN->lpstrFilter = NULL;
    }
}
...
BOOL CFileEditCtrl::FECBrowseForFolder()
{
    ...
    __unaligned ITEMIDLIST *idl = SHBrowseForFolder(m_pBROWSEINFO);
...
//        _tfullpath(File.GetBuffer(_MAX_PATH), lpstrStart, _MAX_PATH); // get absolute path
        _tfullpath(File.GetBuffer(1024), lpstrStart, 1024); // get absolute path //Change MSN 2009-09-24 - Becuase if we have an invalid path that are to long. this can flip out
        File.ReleaseBuffer();
...
void CFileEditCtrl::OnDropFiles(HDROP hDropInfo) 
{
    ...
    for (UINT x = 0; x < nDropCount; x++)
    {
        ...
        {   // first file has complete path, get the length of the path
            ...
            FolderLength = int(_tcslen(Drive) + _tcslen(Path));
        }
        else
...
BOOL CFileEditCtrl::SetFlags(DWORD dwFlags)
{
    ...
    {   // set the control to find files
        ...
        // Added by MSN 2009-01-23

        if(m_szFilename[0] != '\0')
          m_pCFileDialog->m_ofn.lpstrFile = m_szFilename;
