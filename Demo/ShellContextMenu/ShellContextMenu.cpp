...
WNDPROC CShellContextMenu::s_OldWndProc = 0;
...
LRESULT CALLBACK CShellContextMenu::HookWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  return ::CallWindowProc (s_OldWndProc, hWnd, message, wParam, lParam);
}
...
UINT CShellContextMenu::ShowContextMenu(CWnd *pWnd, CPoint pt)
{
  ...
  if(!m_Menu)
  {
	...
	{
		//s_OldWndProc = (WNDPROC) SetWindowLong (pWnd->m_hWnd, GWL_WNDPROC, (DWORD) HookWndProc);
    s_OldWndProc = (WNDPROC)::SetWindowLongPtr(pWnd->m_hWnd, GWLP_WNDPROC, (DWORD_PTR)HookWndProc);
		if (iMenuType == 2)
	...
	if (s_OldWndProc) // unsubclass
  {
		//SetWindowLong (pWnd->m_hWnd, GWL_WNDPROC, (DWORD) m_OldWndProc);
    ::SetWindowLongPtr(pWnd->m_hWnd, GWLP_WNDPROC, (DWORD_PTR)s_OldWndProc);
  }
...
void CShellContextMenu::SetObjects(CStringArray &strArray)
{
	...
	free (olePath);
#else
  CString& s = (CString&)strArray.GetAt(0);
	psfDesktop->ParseDisplayName (NULL, 0, s.GetBuffer (0), NULL, &pidl, NULL);
#endif
